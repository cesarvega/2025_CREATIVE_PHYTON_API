...
from app.constants import TemplateFilename, WordTemplateType
...
_TEMPLATE_MAP: Dict[str, str] = {
    WordTemplateType.NORMAL: TemplateFilename.WORD_RATIONALES,
    WordTemplateType.NORMAL_NO_NEUTRAL: TemplateFilename.WORD_RATIONALES,
    WordTemplateType.KATAKANA: TemplateFilename.WORD_KATAKANA,
    WordTemplateType.KATAKANA_BIG_JAP: TemplateFilename.WORD_KATAKANA,
    WordTemplateType.PHONETICS: TemplateFilename.WORD_PHONETICS,
    WordTemplateType.NONPROPRIETARY: TemplateFilename.WORD_RATIONALES,
    WordTemplateType.TAGLINE: TemplateFilename.WORD_RATIONALES,
    "design": "",  # Keep backward compatibility for design type
}
...
def generate_feedback_document(
    ...
    """Generate a feedback template document for the presentation.
    
    Args:
        presentation_id: The ID of the presentation.
        project_type: The type of project (e.g., "nw", "bsr").
        presentation_type: The presentation type.
        excel_data: Processed Excel data.
        display_name: Display name for the presentation.
        user_name: Name of the user creating the presentation.
        
    Returns:
        Path to the generated document, or empty string if not applicable.
    """
    ...
    template_path = _resolve_template_path(project_type, presentation_type)
    ...
    output_path = _copy_template_to_output(project_type, display_name, template_path)

    if output_path.suffix.lower() == ".docx" and Document is not None:
        _populate_docx(output_path, excel_data)
    else:
        logger.info(
...
def _resolve_template_path(project_type: str, presentation_type: str) -> Optional[Path]:
    """Resolve the template path based on project and presentation types."""
...
def _copy_template_to_output(project_type: str, display_name: str, template_path: Path) -> Path:
...
def _populate_docx(output_path: Path, excel_data: ProcessedExcelData) -> None:
    """Populate a DOCX document with Excel data."""
    if Document is None:
        return
...
class WordService:
    """Deprecated: Use generate_feedback_document() directly."""
    
    def generate_feedback_template(self, *args, **kwargs) -> str:
        """Deprecated: Use generate_feedback_document() directly."""
        return generate_feedback_document(*args, **kwargs)
